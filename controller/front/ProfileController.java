...
public class ProfileController {
    ...
    public String list(Model model, Authentication authentication,@RequestParam(value = "page", defaultValue = DEFAULT_PAGE, required = false) int pageNumber,
        ...
        if (action == null) {
            ...
            model.addAttribute("messageDTO", new MessageDTO(action, status,"Đã hủy đăng ký phòng trọ!"));
        }
        else {
    ...
    public String detailBooking(Model model, @RequestParam long id, @RequestParam(required = false) String action, Authentication authentication) {
        ...
        if (action != null) {
            booking.setProgress("CANCELED");
            bookingService.save(booking);
            String redirectUrl = "/profile?action=cancel&status=success";
